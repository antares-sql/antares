...
ace.define('ace/snippets', ['require', 'exports', 'module', 'ace/lib/oop', 'ace/lib/event_emitter', 'ace/lib/lang', 'ace/range', 'ace/range_list', 'ace/keyboard/hash_handler', 'ace/tokenizer', 'ace/clipboard', 'ace/lib/dom', 'ace/editor'], function (require, exports, module) {
   ...
   (function () {
            ...
            snippetVar: [
               {
                  regex: '\\|' + escape('\\|') + '*\\|',
                  onMatch: function (val, state, stack) {
                     const choices = val.slice(1, -1).replace(/\\[,|\\]|,/g, function (operator) {
      ...
      this.insertSnippetForSelection = function (editor, snippetText) {
         ...
         tokens.forEach(function (p, i) {
            ...
            const isNested = value.some(function (t) {
      ...
      this.expandWithTab = function (editor, options) {
         const self = this;
         const result = editor.forEachSelection(function () {
   ...
   const TabstopManager = function (editor) {
      if (editor.tabstopManager)
...
ace.define('ace/autocomplete/popup', ['require', 'exports', 'module', 'ace/virtual_renderer', 'ace/editor', 'ace/range', 'ace/lib/event', 'ace/lib/lang', 'ace/lib/dom'], function (require, exports, module) {
   ...
   const AcePopup = function (parentNode) {
      ...
      const selectionMarker = new Range(-1, 0, -1, Infinity);
      ...
      const hideHoverMarker = function () {
         setHoverMarker(-1);
      };
      const setHoverMarker = function (row, suppressRedraw) {
      ...
      bgTokenizer.$tokenizeRow = function (row) {
         ...
         if (typeof data === 'string')
            data = { value: data };
         const caption = data.caption || data.value || data.name;
         ...
         const filterText = (popup.filterText || '').toLowerCase();
         let lastIndex = 0;
         let lastI = 0;

         addIconToken(data.meta);

         for (let i = 0; i <= filterText.length; i++) {
            if (i !== lastI && (data.matchMask & (1 << i) || i === filterText.length)) {
               const sub = filterText.slice(lastI, i);
               lastI = i;
               const index = lower.indexOf(sub, lastIndex);
               if (index === -1) continue;
      ...
      popup.goTo = function (where) {
         let row = this.getRow();
         const max = this.session.getLength() - 1;
...
ace.define('ace/autocomplete', ['require', 'exports', 'module', 'ace/keyboard/hash_handler', 'ace/autocomplete/popup', 'ace/autocomplete/util', 'ace/lib/lang', 'ace/lib/dom', 'ace/snippets', 'ace/config'], function (require, exports, module) {
   ...
   (function () {
      ...
      this.updateCompletions = function (keepPopupPosition, options) {
         ...
         if (options && options.matches) {
            const pos = this.editor.getSelectionRange().start;
   ...
   Autocomplete.startCommand = {
      ...
      exec: function (editor, options) {
         const completer = Autocomplete.for(editor);
   ...
   const FilteredList = function (array, filterText) {
      this.all = array;
      this.filtered = array;
   ...
   (function () {
      ...
      this.filterCompletions = function (items, needle) {
         ...
         loop: for (let i = 0, item; item = items[i]; i++) {
            const caption = item.caption || item.value || item.snippet;
            if (!caption) continue;
            let lastIndex = -1;
            let matchMask = 0;
            let penalty = 0;
            ...
            if (this.exactMatch) {
               ...
               const fullMatchIndex = caption.toLowerCase().indexOf(lower);
               if (fullMatchIndex > -1)
